...
- **Simultaneous peer-connect arbitration** (snapshot-22) — inbound and outbound upgraded connections now share the same per-peer settlement table. When two peers dial each other concurrently, peer-ID ordering makes the lower-ID peer keep its initiator connection and the higher-ID peer keep its responder connection, so both endpoints retain the same socket and close the duplicate. Additional same-direction inbound connections remain visible until the dialing peer closes its losing multi-address attempts, avoiding independent endpoint choices that could close every socket.
...
The current source is prepared for this immutable coordinate. Publish it only after the exact commit passes review and verification.

```
community.kotlin.libp2p:jvm-libp2p:1.3.0-codexcoder21-snapshot-22
```
