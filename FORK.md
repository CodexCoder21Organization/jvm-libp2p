...
- **Noise handshake crypto off the event loop** (snapshot-13) — Noise XX Curve25519 DH, Ed25519 signing, and signature verification run on a small shared daemon executor, with a per-channel FIFO preserving handshake-state ordering and a single event-loop task installing the cipher pipeline and replaying early post-handshake frames. This prevents dial storms from freezing established channels that share a `PlainNettyTransport` worker; downstream, 52 providers dialing one relay on two taskset cores left workers RUNNABLE in `Curve25519.eval` while established stream negotiations repeatedly exceeded 1 second for the full 45-second stress window.
