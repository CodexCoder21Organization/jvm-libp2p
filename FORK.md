...
- **Simultaneous peer-connect arbitration** — inbound and outbound upgraded connections now share the same per-peer settlement table. When two peers dial each other concurrently, peer-ID ordering makes the lower-ID peer keep its initiator connection and the higher-ID peer keep its responder connection, so both endpoints retain the same socket and close the duplicate. Additional same-direction inbound connections remain visible until the dialing peer closes its losing multi-address attempts, avoiding independent endpoint choices that could close every socket.
