 ...
 * straight to `shutdownGracefully(0, 5s)`. Netty's event loop still runs the
 * queued bind task before the loop terminates — the port becomes bound — but
 * the channel is now orphaned (never explicitly closed). The OS file descriptor
 ...
 *
 * Downstream this surfaces as `UrlProtocol2.close(): listen port N was not
 * released within 2 seconds of host.stop()` in UrlResolver's
 * `stressTestCloseReleasesPortForDaemonOnlyInstanceUnderLoad` run
 * [`4fe5b865`](https://buildtest.kotlin.build/run?id=4fe5b865), where the
 * eager-join daemon's `host.start()` interleaves with `close()` exactly in
 * this window — close()'s read of an empty map skips the listener teardown,
 * the daemon's bind completes after close()'s shutdownGracefully has begun,
 * and the listen socket stays bound for the lifetime of the test JVM.
 *
 * What this test pins:
